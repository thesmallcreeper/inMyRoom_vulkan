...
SceneNodes::SceneNodes(const tinygltf::Model& in_model, const tinygltf::Scene &in_scene, Anvil::BaseDevice* in_device_ptr)
    :device_ptr(in_device_ptr)
{
    ...
    for(size_t i: in_scene.nodes)
    {
        ...
        else if (!this_node.children.empty())
        {
            ...
            {
                node_and_children_size += AddNode(in_model, in_model.nodes[this_child], node_global_trs_matrix, std::ref(meshes_by_id_TRS));
            }
...
void SceneNodes::Draw(size_t in_primitivesSet_index, Anvil::PrimaryCommandBuffer* in_cmd_buffer_ptr, std::vector<Anvil::DescriptorSet*> in_low_descriptor_sets_ptrs)
{
    ...
    for (const Node& this_node : nodes)
        if (this_node.isMesh)
        {
            nodesMeshes_ptr->Draw(this_node.meshIndex, this_node.meshID, in_primitivesSet_index, in_cmd_buffer_ptr, in_low_descriptor_sets_ptrs);
        }
}

uint32_t SceneNodes::AddNode(const tinygltf::Model& in_model, const tinygltf::Node& in_node, const glm::mat4 parentTRSmatrix, std::vector<glm::mat4>& meshesByIdTRS)
{
    ...
    else if (!in_node.children.empty())
    {
        ...
        {
            nodeAndChildrenSize += AddNode(in_model, in_model.nodes[in_node.children[i]], parent_trs_matrix, std::ref(meshesByIdTRS));
        }
...
glm::mat4 SceneNodes::CreateTRSmatrix(const tinygltf::Node& in_node) const
{
    ...
    else
    {
        ...
        {
            glm::qua<float> rotationQua(in_node.rotation[3], in_node.rotation[0], -in_node.rotation[1], -in_node.rotation[2]);
            return_matrix = glm::toMat4<float,glm::packed_highp>(rotationQua) * return_matrix;
        }
